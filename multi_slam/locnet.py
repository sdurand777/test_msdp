...
from .msdp_dpvo.ba import BA
from .msdp_dpvo.blocks import GatedResidual, GradientClip, SoftAgg
from .msdp_dpvo.lietorch import SE3
from .msdp_dpvo.utils import flatmeshgrid, pyramidify, set_depth
...
class LocNet(nn.Module):
    ...
    def forward(self, images, intrinsics, kp_or_detector, Ps=None, depths=None, bounds=None, M=None, STEPS=2, opt_iters=5, depth_conf=None, use_autocast=False, homography_pretrain=False):
        ...
        for it in range(STEPS):
            ...
            # Inital GUESS POSE ESTIMATION USING EPIPOLAR CONSTRAINTS
            ...
            if (depth_conf is not None) and (it == (STEPS - 1)):
                if isinstance(kp_or_detector, tuple):
                    end_time = time.time()
                    print("LOOP OPTIMISATION POSE : ", end_time - start_time , " seconds")
